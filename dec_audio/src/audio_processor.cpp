...
AudioProcessor::AudioProcessor(ros::NodeHandle node_handle) :
  ...
  num_overlapping_frames_(0), output_start_index_(0), output_end_index_(0), num_selected_output_signals_(0),
  min_freq_(1.0), max_freq_(100.0), freq_status_(diagnostic_updater::FrequencyStatusParam(&min_freq_, &max_freq_)),
  received_first_frame_(false), max_period_between_updates_(0.0), last_callback_duration_(0.0), last_frame_number_(0),
  frame_count_(0), dropped_frame_count_(0), max_dropped_frames_(10),
  consequtively_dropped_frames_(0), publish_visualization_markers_(false),
  visualization_publishing_rate_(0.0), visualization_spectrum_max_amplitude_(0.0), visualization_spectrum_max_height_(0.0),
  spectrum_marker_width_(0.0),
  user_callback_enabled_(false), recording_(false)
{
}
...
bool AudioProcessor::initialize(bool set_auto_update)
{
  ...
  ROS_VERIFY(dec_utilities::read(node_handle_, "spectrum_base_frame_pose", spectrum_base_frame_pose_));
...
bool AudioProcessor::initializeAudio()
{
  ...
  int sound_card_id;
  ROS_VERIFY(dec_utilities::read(node_handle_, "sound_card_id", sound_card_id));
  int device_id;
  ROS_VERIFY(dec_utilities::read(node_handle_, "device_id", device_id));
...
//  }
  ...
  {
    ROS_ERROR("Missed >%i< consecutive frames, which is more than allowed >%i<.", consequtively_dropped_frames_, max_dropped_frames_);
    ROS_BREAK();
  }
...
{
  ROS_VERIFY(dec_utilities::read(node_handle_, "output_sample_rate", output_sample_rate_));
  ROS_ASSERT(output_sample_rate_ > 0);
  ROS_VERIFY(dec_utilities::read(node_handle_, "num_channels", num_channels_));

  ROS_VERIFY(dec_utilities::read(node_handle_, "num_frames_per_period", num_frames_per_period_));
  ...
  ROS_VERIFY(dec_utilities::read(node_handle_, "num_output_signals", num_output_signals_));
  ...
  ROS_VERIFY(dec_utilities::read(node_handle_, "output_start_index", output_start_index_));
  ROS_VERIFY(dec_utilities::read(node_handle_, "output_end_index", output_end_index_));
  ROS_ASSERT(output_start_index_ < output_end_index_);
  ROS_ASSERT(output_end_index_ <= num_output_signals_);
  num_selected_output_signals_ = output_end_index_ - output_start_index_;
  ...
  ROS_VERIFY(dec_utilities::read(node_handle_, "publish_visualization_markers", publish_visualization_markers_));

  double visualization_publishing_rate;
  ROS_VERIFY(dec_utilities::read(node_handle_, "visualization_publishing_rate", visualization_publishing_rate));
  ...
  ROS_VERIFY(dec_utilities::read(node_handle_, "visualization_spectrum_max_amplitude", visualization_spectrum_max_amplitude_));
  ROS_ASSERT(visualization_spectrum_max_amplitude_ > 0);
  ROS_VERIFY(dec_utilities::read(node_handle_, "visualization_spectrum_max_height", visualization_spectrum_max_height_));
  ROS_ASSERT(visualization_spectrum_max_height_ > 0);

  ROS_VERIFY(dec_utilities::read(node_handle_, "apply_dct", apply_dct_));

  ROS_VERIFY(dec_utilities::read(node_handle_, "mel_filter_parameter_a", mel_filter_parameter_a_));
  ROS_VERIFY(dec_utilities::read(node_handle_, "mel_filter_parameter_b", mel_filter_parameter_b_));
  ROS_VERIFY(initMelFilterBank());

  std::vector<double> output_scaling;
  if(apply_dct_)
  {
    ROS_VERIFY(dec_utilities::read(node_handle_, "dct_output_scaling", output_scaling));
  }
  else
  {
    ROS_VERIFY(dec_utilities::read(node_handle_, "no_dct_output_scaling", output_scaling));
  }
  ...
  if(apply_dct_)
  {
    ROS_VERIFY(dec_utilities::read(node_handle_, "dct_output_offset", output_offset));
  }
  else
  {
    ROS_VERIFY(dec_utilities::read(node_handle_, "no_dct_output_offset", output_offset));
  }
...
//bool AudioProcessor::setBackgroundNoise(dec_audio::SetBackgroundNoise::Request& request,
//                                        dec_audio::SetBackgroundNoise::Response& response)
//{
//  background_noise_mutex_.lock();
...
bool AudioProcessor::dumpRawAudio(dec_audio::DumpRawAudio::Request& request,
                                  dec_audio::DumpRawAudio::Response& response)
{
