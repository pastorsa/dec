...
Adafruit_NeoPixel::Adafruit_NeoPixel()
  : is_setup_(0), num_leds_(0), num_bytes_(0), pixels_(NULL), pin_(0), pin_mask_(0),
    type_(NEO_GRB + NEO_KHZ800), port_(NULL), end_time_(0)
{
}
...
void Adafruit_NeoPixel::setup(uint16_t n, uint8_t p, uint8_t t)
{
  ...
  {
    is_setup_ = 1;
    memset(pixels_, 0, num_bytes_);
    num_leds_ = n;
    type_ = t;
    pin_ = p;
    port_ = portOutputRegister(digitalPinToPort(p));
    pin_mask_ = digitalPinToBitMask(p);
    end_time_ = 0L;
  }
  else
  {
...
void Adafruit_NeoPixel::show(void)
{
  ...
  volatile uint16_t i = num_bytes_; // Loop counter
  volatile uint8_t *ptr = pixels_, // Pointer to next byte
  ...
  cli();
  // Disable interrupts; need 100% focus on instruction timing

#ifdef __AVR__

#if (F_CPU == 8000000UL) // FLORA, Lilypad, Arduino Pro 8 MHz, etc.
  if((type_ & NEO_SPDMASK) == NEO_KHZ800)
  { // 800 KHz bitstream
  ...
  if ((type_ & NEO_SPDMASK) == NEO_KHZ400)
  { // 400 KHz bitstream
...
#error "24 MHz not supported, use Tools > CPU Speed at 48 or 96 MHz"
#endif
  uint8_t *p = pixels_;
  uint8_t *end = pixels_ + num_bytes_;
  volatile uint8_t *set = portSetRegister(pin_);
  volatile uint8_t *clr = portClearRegister(pin_);
  if ((type_ & NEO_SPDMASK) == NEO_KHZ800)
  { // 800 KHz bitstream
    while (p < end)
    {
  ...
  end_time_ = micros(); // Note EOD time for latch on next call
}
...
void Adafruit_NeoPixel::setPixelColor(uint16_t n, uint8_t r, uint8_t g, uint8_t b)
{
  if (n < num_leds_)
  {
    uint8_t *p = &pixels_[n * 3];
    if ((type_ & NEO_COLMASK) == NEO_GRB)
    {
      *p++ = g;
      *p++ = r;
    }
...
uint32_t Adafruit_NeoPixel::getPixelColor(uint16_t n)
{
  ...
  {
    uint16_t ofs = n * 3;
    return (uint32_t)(pixels_[ofs + 2])
        | (((type_ & NEO_COLMASK) == NEO_GRB) ? ((uint32_t)(pixels_[ofs]) << 8) | ((uint32_t)(pixels_[ofs + 1]) << 16) :
            ((uint32_t)(pixels_[ofs]) << 16) | ((uint32_t)(pixels_[ofs + 1]) << 8));
  }
